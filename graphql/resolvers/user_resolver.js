...
export const userResolvers = {
  Mutation: {
    async registerUser(_, { registerInput: { username, email, password, confirmPassword } }) {
      console.log("Input Values:", { username, email, password, confirmPassword });

      const { errors, isValid } = validateRegisterInput(username, email, password, confirmPassword);
      console.log("Validation Results:", { errors, isValid });
      ...
      if (oldUser) {
        throw new UserInputError("User with this email is already registered", {
          errors: {
    ...
    async loginUser(_, {loginInput:{ email, password}}){
        ...
        if(!matchPassword){
            ...
            throw new UserInputError("Wrong crendtials", {
              errors
            });
        }
